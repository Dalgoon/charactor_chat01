...
import defaultHayoonseoPrompt from '../../[하윤서]/하윤서 시스템 프롬프트.txt?raw';
import defaultHayoonseoPrologue from '../../[하윤서]/하윤서 프롤로그.txt?raw';

import defaultChaeharinPrompt from '../../[채하린]/채하린 시스템 프롬프트.txt?raw';
import defaultChaeharinPrologue from '../../[채하린]/채하린 프롤로그.txt?raw';
import chaeharinTagsData from '../../[채하린]/채하린, 이미지 태그 및 url.txt?raw';
...
export const hayoonseoCharacter = {
  id: 'char_hayoonseo',
  name: '하윤서',
  avatar: 'https://3nio.com/NI/A/02.webp',
  model: 'gemini-2.5-flash',
  maxImages: 1,
  systemPrompt: defaultHayoonseoPrompt,
  greeting: defaultHayoonseoPrologue,
  imageMap: [], // URL is dynamically generated by AI
  messages: [
    {
      id: "prologue_msg_hayoonseo_1",
      role: 'model',
      text: defaultHayoonseoPrologue,
      timestamp: Date.now(),
      situationUrl: 'https://3nio.com/NI/B/12.webp' // 12:놀람
    }
  ]
};
...
export const loadCharacters = () => {
  ...
  if (!data) {
    const initialChars = [...defaultCharacters, bulkCharacter, pengkoCharacter, hayoonseoCharacter, chaeharinCharacter];
  ...
  // 기존 유저의 로컬 스토리지 데이터에 하윤서가 없다면 자동 추가
  const hayoonseo = parsed.find(c => c.id === 'char_hayoonseo');
  if (!hayoonseo) {
    parsed.push(hayoonseoCharacter);
    changed = true;
  } else {
    // 하윤서의 시스템 프롬프트를 텍스트 파일의 내용으로 강제 고정
    hayoonseo.systemPrompt = defaultHayoonseoPrompt;
    hayoonseo.greeting = defaultHayoonseoPrologue;
    if (hayoonseo.messages.length > 0 && hayoonseo.messages[0].id === "prologue_msg_hayoonseo_1") {
      hayoonseo.messages[0].text = defaultHayoonseoPrologue;
    }
    changed = true;
  }

  // 기존 유저의 로컬 스토리지 데이터에 채하린이 없다면 자동 추가
  const chaeharin = parsed.find(c => c.id === 'char_chaeharin');
  if (!chaeharin) {
    parsed.push(chaeharinCharacter);
    changed = true;
  } else {
    // 채하린의 시스템 프롬프트를 텍스트 파일의 내용으로 강제 고정
    chaeharin.systemPrompt = defaultChaeharinPrompt;
    chaeharin.greeting = defaultChaeharinPrologue;
    chaeharin.imageMap = chaeharinCharacterMap;
    if (chaeharin.messages.length > 0 && chaeharin.messages[0].id === "prologue_msg_chaeharin_1") {
      chaeharin.messages[0].text = defaultChaeharinPrologue;
    }
    changed = true;
  }
...
export const parseSituationFromText = (text, character) => {
  ...
  if (matches.length === 0 && directUrlMatches.length === 0) {
    console.log(`❌ AI가 텍스트 안에 [상황태그] 또는 URL 형태의 텍스트를 단 하나도 출력하지 않았습니다.`);
    console.log(`=========================================\n`);
  } else {
